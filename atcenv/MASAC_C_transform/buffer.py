...
class ReplayBuffer:
    ...
    def __init__(self, obs_dim: int, action_dim: int, n_agents: int, size: int, batch_size: int = 32):
        ...
        self.qobs_buf = np.zeros([size, n_agents, 14], dtype=np.float32)
        self.qnext_obs_buf = np.zeros([size, n_agents, 14], dtype=np.float32)
    ...
    def store(
        ...
        rew: float,
        next_obs: np.ndarray,
        qnext_obs: np.ndarray,
        done: bool,
    ) -> Tuple[np.ndarray, np.ndarray, float, np.ndarray, bool]:
        """ Store transition """
        self.obs_buf[self.ptr] = obs
        self.qobs_buf[self.ptr] = qobs
        self.acts_buf[self.ptr] = act
        self.rews_buf[self.ptr] = rew
        self.next_obs_buf[self.ptr] = next_obs
        self.qnext_obs_buf[self.ptr] = qnext_obs
    ...
    def sample_batch(self) -> Dict[str, np.ndarray]:
        ...
        return dict(obs = self.obs_buf[idx],
            qobs = self.qobs_buf[idx],
            next_obs = self.next_obs_buf[idx],
            qnext_obs = self.qnext_obs_buf[idx],
